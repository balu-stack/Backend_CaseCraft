...
def start_case(req: VerifyPatientCaseRequest):
    ...
    try:
        ...
        if not patient_row:
            return {
                "status": "fail",
                "message": f"Patient not found in your clinic. patient_id={patient_id}, clinic_id={clinic_id}"
            }
    ...
    try:
        cur.execute(
            ...
            SELECT
                ...
                c.created_at
            FROM cases c
            LEFT JOIN patients p ON UPPER(TRIM(c.patient_id)) = UPPER(TRIM(p.patient_id))
...
# ✅ NEW: get all cases for clinic
@router.get("/clinic/{clinic_id}")
def get_cases_for_clinic(clinic_id: str):
    ...
    try:
        cur.execute(
            ...
            SELECT
                ...
                c.updated_at
            FROM cases c
            WHERE UPPER(TRIM(c.clinic_id)) = %s
            ORDER BY c.updated_at DESC, c.created_at DESC
            """,
            (cid,)
        )
...
def delete_case(case_id: str):
    ...
    try:
        cur.execute(
            "SELECT case_id FROM cases WHERE UPPER(TRIM(case_id))=%s",
            (cid,)
        )
        row = cur.fetchone()

        if not row:
            return {"status": "fail", "message": "Case not found"}

        cur.execute(
            "DELETE FROM cases WHERE UPPER(TRIM(case_id))=%s",
            (cid,)
        )
        conn.commit()
