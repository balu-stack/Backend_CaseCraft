...
def clinic_verify_otp(req: ClinicVerifyOtpRequest):
    ...
    try:
        ...
        cur.execute(
            """
            SELECT clinic_id, clinic_name, clinic_email, clinic_phone
            FROM clinics
            WHERE clinic_id=%s
            """,
            (clinic_id,),
        ...
        if not clinic_row:
            return {"status": "fail", "message": "Clinic not found"}

        real_clinic_id, clinic_name, clinic_email, clinic_phone = clinic_row
