    ...
    try:
        ...
        cur.execute(
            ...
            INSERT INTO case_forms (
                ...
                lingual_frenum, dental_status, perio_chart
            )
            VALUES (
                %s,
            ...
            ON DUPLICATE KEY UPDATE
                ...
                perio_chart=VALUES(perio_chart),
        ...
        # Draft save => in_progress
        # Final submit => completed
        if req.is_final_submit:
            cur.execute(
                """
                UPDATE cases
            ...
            message = "Draft auto-saved successfully as in_progress"

        conn.commit()
        return SimpleCaseFormResponse(status="success", message=message)
